...
public class GiantAI : MonoBehaviour {
    ...
    private void OnCollisionStay2D(Collision2D collision)
    {
		...
		if (collision.gameObject.name == "Player" && damageTimer == 0)
		{
			collision.gameObject.GetComponent<GameControl>().healthcount -= damage;
			damageTimer = damageDelay;
		}

		if (collision.gameObject.name == "Lost Tower" && damageTimer == 0)
		{
            //collision.gameObject.GetComponent<MainTower>().healthcount -= 1;
			MainTower.healthcount -= damage;
			damageTimer = damageDelay;
		}
			
        //Debug.Log(collision.gameObject.name);
        if (collision.gameObject.tag == "building" && damageTimer == 0)
        {
            if (collision.gameObject.name == "Tower 05(Clone)")
            {
				collision.gameObject.GetComponent<TankTower>().health -= damage;

                damageTimer = damageDelay;

                print("ATTACK");
            }
            else if (collision.gameObject.name == "Tower 04(Clone)")
            {
				collision.gameObject.GetComponent<ResourceTower>().health -= damage;

                damageTimer = damageDelay;

                print("ATTACK");
            }
            else if (collision.gameObject.name == "Tower 02(Clone)")
            {
				collision.gameObject.GetComponent<LongTower>().health -= damage;

                damageTimer = damageDelay;

                print("ATTACK");
            }
            else if (collision.gameObject.name == "Tower 01(Clone)")
            {
				collision.gameObject.GetComponent<TowerFire>().health -= damage;
